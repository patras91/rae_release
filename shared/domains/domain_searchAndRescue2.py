...
Rescue_Method2.parameters = "sorted([(r2,) for r2 in rv.WHEELEDROBOTS if state.status[r2] == 'free' and SR_GETDISTANCE_Euclidean(state.loc[r2], (1,1)) < dist], key = lambda x: SR_GETDISTANCE_Euclidean(state.loc[x], (1,1)))"
...
def SurveyAll_Method1(r):
    ...
    for i in range(30):
        for j in range(30):
            ...
            alg.do_command(captureImage, r, 'bottomCamera', (i,j))
    ...
    alg.add_tasks([['rescue', r, person] for person in persons])
...
from env_searchAndRescue2 import *