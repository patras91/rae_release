...
def Populate(res, domain, simmode):
    for k in K[domain]:
        if k == 0:
            f_rae_name = "outputs1/{}/APE.txt".format(domain)
            f_rae = open(f_rae_name, "r")
            PopulateHelper(res, domain, f_rae, k)
            f_rae.close()
        else:
            if simmode == 'active':
                fname = 'outputs2/{}/plan{}.txt'.format(domain, k)
            elif simmode == 'lazy':
                fname = 'outputs2/{}/plan{}.txt'.format(domain, k)
...
def PlotNu(resDict):
    ...
    for domain in resDict:
        ...
        fname = 'figures/Nu{}.png'.format(domain)
        plt.xlabel('k1')
        plt.ylabel('Speed $\\nu$') # for {}'.format(domain))
...
def PlotNuToppedSep(resDict):
    ...
    fname = 'figures/NuNoDeadEnds.png'
    plt.savefig(fname, bbox_inches='tight')
...
def PlotNuDet(resDict):
    ...
    for domain in resDict:
        ...
        fname = 'figures/Nu{}points.png'.format(domain)
        plt.savefig(fname, bbox_inches='tight')
...
def PlotCommandTotals(resDict):
    ...
    fname = 'figures/CommandTotals.png'
    plt.xlabel('k1')
...
def PlotSuccessCounts(resDict):
    ...
    fname = 'figures/SuccessWithRetryPenalty.png'
    plt.savefig(fname, bbox_inches='tight')
...
def PlotRetrySideBySide(resDict):
    ...
    #fig = plt.gcf()
    #fig.set_size_inches(40, 10)
    fname = 'figures/RetriesAllDomains.png'
    plt.savefig(fname, bbox_inches='tight')
...
def PlotSuccessTopped(resDict):
    ...
    fname = 'figures/SuccessAllDomains.png'
    plt.savefig(fname, bbox_inches='tight')
...
def PlotRetryToppedSep(resDict):
    ...
    val = [0, 1, 2, 3, 4, 5]
    l = 6
    plt.bar(EditToFitBarPlot(val, 0 * width), resDict['EE'][label1][index1][0:l], width=width, edgecolor='black', hatch="//", label='EE', color=color1, linewidth=2)
    plt.bar(EditToFitBarPlot(val, 1.25 * width), resDict['CR'][label1][index1][0:l], width=width, edgecolor='black', hatch="...", label='CR', tick_label=val, color='black', linewidth=2)
    ...
    fname = 'figures/RetriesDeadEnds.png'
    plt.savefig(fname, bbox_inches='tight')
    ...
    fname = 'figures/RetriesNoDeadEnds.png'
    plt.savefig(fname, bbox_inches='tight')
...
def PlotRetryInv(resDict):
    ...
    fname = 'figures/RetryInvAllDomains.png'
    plt.savefig(fname, bbox_inches='tight')
...
def PlotSimTimes(resDict):
    ...
    fname = 'figures/SimulationTimes.png'
    plt.xlabel('k1')
...
def PlotRunningTimes(resDict):
    ...
    fname = 'figures/RunningTimes.png'
    plt.xlabel('k1')
