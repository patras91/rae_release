...
class Room:
    ...
    def get_random_pos_inside(self):
        # Added this for the KeyCorridorGBLA domain
        topX, topY = self.top
        sizeX, sizeY = self.size
        x = random.randint(topX + 1, topX + sizeX - 1)
        y = random.randint(topY + 1, topY + sizeY - 1)
        return (x, y)
...
class RoomGrid(MiniGridEnv):
    ...
    def get_loc_inside_room(self, i, j):
        room = self.get_room(i, j)
        return room.get_random_pos_inside()
