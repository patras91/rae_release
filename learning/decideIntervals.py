...
def EncodeState_CR(state):
	...
	a1 = a[0][4:]
	a1_int = ConvertToInt(GetNums(a1))
	a1H = []
	for i in a1_int:
		a1H += ConvertToOneHot(i,UpperLimits['CR']['loc'])[1:]
	
	a2 = a[1][6:]
	a2H = []
	a2_int = ConvertToInt(GetNums(a2))
	for i in a2_int:
		a2H += ConvertToOneHot(i,UpperLimits['CR']['charge'])

	a3 = a[2][5:]
	a3H = []
	a3_int = ConvertToInt(GetLoadString(a3))
	for i in a3_int:
		a3H += ConvertToOneHot(i,UpperLimits['CR']['load'])

	a4 = a[3][4:]
	a4H = []
	a4_int = ConvertToInt(GetPosString(a4))
	for i in a4_int:
		a4H += ConvertToOneHot(i,UpperLimits['CR']['pos'])

	a5 = a[5][18:]
	a5H = []
	a5_int = ConvertToInt(GetEmS(a5))
	for i in a5_int:
		a5H += ConvertToOneHot(i,UpperLimits['CR']['emergencyHandling'])

	a6 = a[6][5:]
	a6H = []
	a6_int = ConvertToInt(GetViewString(a6))
	for i in a6_int:
		a6H += ConvertToOneHot(i,UpperLimits['CR']['view'])
...
def EncodeForHeuristic(domain, state, method):
	if domain == "CR":
		x = EncodeState_CR(state)
	elif domain == "SR":
		x = EncodeState_SR(state)
	...
	methodCode = ConvertToOneHot(methodCodes[domain][method], 10)
	x += methodCode
	return x
...
if __name__ == "__main__":
	...
	while(line != ""):
		if domain == "CR":
			ReadStateVars_CR(line, f)
		elif domain == "SR":
			ReadStateVars_SR(line, f)
		elif domain == "SD":
			ReadStateVars_SD(line, f)
		elif domain == "EE":
			ReadStateVars_EE(line, f)
